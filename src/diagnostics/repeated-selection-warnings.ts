...
const UNKNOWN_SELECTION_LOCATION = '<unknown location>'

const getSelectionSetLabel = (definition: FragmentDefinitionNode | OperationDefinitionNode): string => {
    ...
    selectionSet.selections.forEach(selection => {
        if (selection.kind === Kind.FIELD) {
            ...
            if (fieldLocations.has(responseName)) {
                console.warn(
                    `Repeated field selection "${responseName}" detected in ${selectionSetLabel} at "${selectionLocation ?? UNKNOWN_SELECTION_LOCATION}". `
                    + `The plugin merged it, but the selection is redundant. First occurrence: "${fieldLocations.get(responseName)}".`
                )
            } else {
                fieldLocations.set(responseName, selectionLocation ?? UNKNOWN_SELECTION_LOCATION)
            }
        }
        ...
        if (selection.kind === Kind.FRAGMENT_SPREAD) {
            ...
            if (spreadLocations.has(selection.name.value)) {
                console.warn(
                    `Repeated fragment spread "${selection.name.value}" detected in ${selectionSetLabel} at "${selectionLocation ?? UNKNOWN_SELECTION_LOCATION}". `
                    + `The plugin merged it, but the spread is redundant. First occurrence: "${spreadLocations.get(selection.name.value)}".`
                )
            } else {
                spreadLocations.set(selection.name.value, selectionLocation ?? UNKNOWN_SELECTION_LOCATION)
            }
        }
    })
}
