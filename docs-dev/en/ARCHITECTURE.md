...
- Publication rejects unowned files and files modified after generation by
  default. `generateDeclarations(config, { force: true })` overwrites planned
  output paths regardless of ownership and removes stale manifest-owned files.
  Other files are preserved and reported in `result.warnings`; they are not
  claimed in the new manifest. Only stale files from the previous manifest may be deleted.
