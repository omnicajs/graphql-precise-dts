...
`generateDeclarations(config, options?)` проверяет конфигурацию, затем захватывает
блокировки проектов в стабильном порядке. Файлы блокировок находятся в
`.graphql-precise-dts/locks/` относительно `config.root` по умолчанию.
`locks.directory` меняет этот путь: относительные пути считаются от `config.root`,
абсолютные используются напрямую. Конкурирующие запуски одного проекта должны
использовать один каталог блокировок. Блокировка содержит PID
...
2. Если `options.force` не равен `true`, проверяет хеши ранее опубликованных
   файлов и отклоняет чужие файлы на запланированных путях. Конфликт останавливает
   публикацию до первой записи. Force разрешает перезапись этих путей и ищет
   незаявленные файлы в выходных каталогах, чтобы сохранить их и предупредить.
3. Записывает файлы через временный файл и `rename`. Если содержимое совпадает,
   запись пропускается и timestamp сохраняется.
4. Удаляет устаревшие файлы, принадлежащие предыдущей публикации, в том числе
   изменённые, если включён force.
5. Записывает manifests последними.

В CLI опция этого вызова доступна как `generate --force`; команды `check` и
`list` её отклоняют. Force не обходит диагностики уровня `error`, валидацию
manifest. Сохранённые незаявленные файлы не попадают в новый manifest и
возвращаются в `result.warnings` как записи `dirty-output` с `root` и
отсортированными путями `files`. CLI выводит их в stderr без завершения с ошибкой.
Вложенные выходные каталоги сканируются отдельно, символические ссылки на
каталоги не обходятся. Политика кеша схемы не меняется.
