...
- Публикация никогда не перезаписывает чужие файлы. По умолчанию она также
  отклоняет файлы, изменённые после записи генератором.
  `generateDeclarations(config, { force: true })` разрешает перезапись изменённых
  файлов генератора и удаление изменённых устаревших файлов. Удалять можно только
  устаревшие файлы из предыдущего manifest.
