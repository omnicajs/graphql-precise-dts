...
describe('fragments', () => {
    describe('UserDetails', () => {
        ...
        test('compatible with query result', async () => {
            ...
            data!.groupMembers!.map(gm => {
                expectTypeOf(gm).toExtend<UserDetailsFragment>()
                expectTypeOf<UserDetailsFragment>().toExtend<typeof gm>()
            })
        })
    })
    ...
    describe('GroupDetails', () => {
        ...
        test('compatible with query result', async () => {
            ...
            data!.userGroups!.map(g => {
                expectTypeOf(g).toExtend<GroupDetailsFragment>()
                expectTypeOf<GroupDetailsFragment>().toExtend<typeof g>()
    ...
    describe('UserWithGroupsDetails', () => {
        test('signature match', () => {
            type Expected = UserDetailsFragment & { groups: Array<GroupDetailsFragment> }

            // Intersections may differ syntactically while accepting exactly the same values.
            expectTypeOf<UserWithGroupsFragment>().toExtend<Expected>()
            expectTypeOf<Expected>().toExtend<UserWithGroupsFragment>()
        })
        ...
        test('compatible with query result', async () => {
            ...
            expectTypeOf(user).toExtend<UserWithGroupsFragment>()
            expectTypeOf<UserWithGroupsFragment>().toExtend<typeof user>()
        })

        test('fragment fields correspond to the signatures of nested fragments', () => {
            expectTypeOf<UserWithGroupsFragment>().toMatchObjectType<UserDetailsFragment>()
            expectTypeOf<Element<UserWithGroupsFragment['groups']>>().toExtend<GroupDetailsFragment>()
            expectTypeOf<GroupDetailsFragment>().toExtend<Element<UserWithGroupsFragment['groups']>>()
        })
    })
})
