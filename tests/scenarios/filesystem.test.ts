...
describe('experimental filesystem public API', () => {
    ...
    test('force regenerates modified owned outputs and removes modified stale files', async () => {
        const root = resolve(fixturesRoot, 'publication')
        const config = defineConfig({
            root,
            schemas: {
                core: {
                    file: 'schemas/core/schema.graphql',
                    typesModule: '@app/graphql/schema',
                    outputs: { root: 'published/force/schema', types: 'schema.d.ts' },
                },
            },
            projects: {
                app: {
                    ...
                    targets: {
                        core: {
                            schema: 'core',
                            documents: { files: ['queries/users.graphql'] },
                            outputs: { tree: { root: 'published/force/app' } },
                        },
                    },
                },
            },
        })
        const previous = await generateDeclarations({
            ...config,
            schemas: {
                core: {
                    ...config.schemas.core,
                    enumsModule: '@app/graphql/enums',
                    outputs: { ...config.schemas.core.outputs, enums: 'enums.ts' },
                },
            },
            projects: {
                app: {
                    ...
                    targets: {
                        core: {
                            ...config.projects.app.targets.core,
                            documents: { files: ['queries/users.graphql', 'queries/status.graphql'] },
                        },
                    },
                },
            },
        ...
        for (const output of previous.outputs) {
            writeFileSync(resolve(root, output.file), 'generated by another version')
        }
        const foreignFile = resolve(root, 'published/force/schema/notes.txt')
        writeFileSync(foreignFile, 'not generated')

        await expect(generateDeclarations(config, { force: false }))
            .rejects.toThrow('was modified after publication')
        const result = await generateDeclarations(config, { force: true })

        expect(result.diagnostics).toEqual([])
        expect(readFileSync(resolve(root, 'published/force/schema/schema.d.ts'), 'utf8'))
            .toBe(readFixture('publication/expected/schema.d.ts'))
        expect(readFileSync(resolve(root, 'published/force/app/queries/users.graphql.d.ts'), 'utf8'))
            .toBe(readFixture('publication/expected/app/queries/users.graphql.d.ts'))
    ...
    test.each([false, true])('keeps the last successful declaration tree on compilation errors with force=%s', async force => {
        const root = resolve(fixturesRoot, 'publication')
        ...
        const failed = await generateDeclarations(config, { force })
